...
export function checkAbapSource(source, opts = {}) {
  ...
  /* Two builders, one node tree: the typed z2ui5_cl_xml_view (what most
   * abap2UI5 apps are written with) and the generic z2ui5_cl_ai_xml. Both
   * reconstructions produce the same shape, so every gate below is unaware
   * of which one was used. */
  const typed = usesTypedBuilder(source);
  const prep = typed ? prepareTypedAbap(source) : prepareAbap(source);
  const result = {
    kind: 'abap',
    builder: typed ? 'typed' : 'generic',
...
export function collectFiles(paths) {
  ...
  const visit = (p) => {
    ...
    if (p.endsWith('.clas.abap') && !p.endsWith('.testclasses.abap')) {
      ...
      if (src.includes('z2ui5_cl_ai_xml=>factory') || src.includes('z2ui5_cl_xml_view=>factory')) {
        out.push(p);
      }
    }
  };
