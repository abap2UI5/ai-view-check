...
export function checkNodes(root, { data, minUi5 = '1.71', allow = [], distribution = 'sapui5', model = null } = {}) {
  ...
  const pathInModel = (p) => hasPath(model, p);
  ...
  (function walk(node, ownerControl, ctx) {
    ...
    if (node.name !== null && !isAggregationTag(node.name)) {
      ...
      if (full && knownControl(data, full)) {
        ...
        if (model) {
          for (const [attr, value] of node.attrs) {
            ...
            const abs = bindingPath(value);
            const rel = abs ? null : relativePath(value);
            const bound = abs ? valueAt(model, abs)
              : rel && ctx ? valueAt(ctx.row, rel) : undefined;
            const row = templateRow(bound);
            if (row) aggRows[attr] = { row, path: abs || rel };
          }
        }
        ...
        childCtx = (defAgg && aggRows[defAgg]) || ctx;
      }
      ...
      childCtx = ownerControl.aggRows?.[node.name] ?? ownerControl.ctx ?? null;
    }
    for (const c of node.children) walk(c, owner, childCtx);
  })(root, null, null);
