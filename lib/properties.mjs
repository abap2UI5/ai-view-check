 ...
 */
export function checkNodes(root, { data, minUi5 = '1.71', allow = [], distribution = 'sapui5', model = null } = {}) {
  ...
  (function walk(node, ownerControl) {
    ...
    if (node.name !== null && !isAggregationTag(node.name)) {
      ...
      if (node.ns && !(node.ns in nsMap)) {
        report({ type: 'undeclared-namespace', control: node.name, member: node.ns });
      }
      ...
      if (full && knownControl(data, full)) {
        ...
        if (idAttr && !isBinding(idAttr[1])) {
          if (idsSeen.has(idAttr[1])) {
            report({ type: 'duplicate-id', control: full, member: 'id', value: idAttr[1] });
          }
        ...
        for (const [attr, value] of node.attrs) {
          ...
          if (member.set === 'events' && !isHandler && isBinding(value)) {
            report({ type: 'binding-for-event', control: full, member: attr });
          } else if (member.set !== 'events' && isHandler) {
            report({ type: 'event-for-property', control: full, member: attr });
          }

          if (brokenExpression(value)) {
            report({ type: 'invalid-expression-binding', control: full, member: attr, value: String(value).slice(0, 80) });
          }

          // a bound path the derived model does not have - a silent empty
          // field; only absolute paths are resolvable without a context
          if (model) {
            const bp = bindingPath(value);
            if (bp && !pathInModel(bp)) {
              report({ type: 'unknown-binding-path', control: full, member: attr, value: bp });
            }
          }
          ...
          if (member.set === 'properties' && model) {
            const bp = bindingPath(value);
            if (bp) {
              let cur = model;
        ...
        for (const rule of REQUIRED_WITH) {
          ...
          if (bound && !has) {
            report({ type: 'missing-required-aggregation', control: full, member: rule.needs });
          }
        }
        ...
        for (const child of node.children) {
          ...
          if (aggHere.has(child.name)) {
            report({ type: 'duplicate-aggregation', control: full, member: child.name });
          }
        ...
        if (isA(data, full, 'sap.m.Button') && attrOf('icon')
          ...
          report({ type: 'missing-accessibility', control: full, member: 'tooltip' });
        }
        if (isA(data, full, 'sap.m.Image') && !attrOf('alt') && !attrOf('decorative')) {
          report({ type: 'missing-accessibility', control: full, member: 'alt' });
        }
