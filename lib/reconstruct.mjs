...
function parseRows(region, rowType, types) {
  ...
  return rows.map((rowSrc) => {
    const row = Object.fromEntries(fields.map((f) => [up(f.name), scalarDefault(f.type)]));
    if (complete) Object.defineProperty(row, '__complete', { value: true });
    ...
    if (decl.kind === 'table') {
      ...
      model[up(v)] = tableSeed.has(v)
        ...
        : types.structs.has(rowType)
          ? [markComplete(Object.fromEntries(
            types.structs.get(rowType).map((f) => [up(f.name), scalarDefault(f.type)])))]
...
export function prepareAbap(abapSource) {
  ...
  return {
    nodes, docs, model, notes, helperTokens,
