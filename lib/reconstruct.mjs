...
export function makeResolver(content, boundVars, notes) {
  ...
  const resolveOne = (piece) => {
    ...
    /* The typed builder takes ABAP booleans directly and renders abap_true
     * as "true" (xml_get_parts); abap_false is initial and drops out. */
    if (/^abap_true$/i.test(s)) return 'true';
    if (/^abap_false$/i.test(s)) return '';
...
function applyToken(verb, body, stack, resolveExpr, notes, structure, offset) {
  ...
  if (am) {
    ...
    for (const kv of region.matchAll(/`((?:[^`]|``)*)`/g)) {
      ...
      const eq = s.indexOf('=');
      if (eq > 0) addAttr(node, s.slice(0, eq).trim(), s.slice(eq + 1), offset, structure);
    }
  }
  const cur = stack[stack.length - 1];
 ...
 *
 * Shared with the typed builder (xml-view.mjs): the model comes from the
 * class's TYPES/DATA and its seeds, which are the same either way. */
